...
export class TripService {
    ...
    async searchTrips(userId, filters) {
        const user = await prisma.user.findUnique({
            where: { id: userId },
        ...
        const blockedUserIds = new Set(blocks.flatMap(b => [b.blockerId, b.blockedId]).filter(id => id !== userId));
    ...
    async getNearbyTrips(userId, filters) {
        ...
        const tripsWithDistance = trips.map(trip => {
            const distance = this.calculateDistance(filters.lat, filters.lng, Number(trip.originLat), Number(trip.originLng));
            return {
                ...this.formatTrip(trip),
    ...
    async updateTrip(tripId, userId, data) {
        ...
        if (data.totalSeats && data.totalSeats < trip._count.matches) {
            throw new AppError(`Cannot reduce seats below ${trip._count.matches} confirmed participants`, 400);
        }
    ...
    // 3.6 Cancel Trip
    async cancelTrip(tripId, userId, data) {
        const trip = await prisma.trip.findUnique({
            where: { id: tripId },
            include: {
                matches: { where: { status: 'accepted' } },
        ...
        departureDateTime.setHours(trip.departureTime.getHours(), trip.departureTime.getMinutes());
    ...
    async markTripInProgress(tripId, userId) {
        const trip = await prisma.trip.findUnique({
            where: { id: tripId },
            include: {
    ...
    async markTripCompleted(tripId, userId) {
        const trip = await prisma.trip.findUnique({
            where: { id: tripId },
            include: {
                matches: { where: { status: 'accepted' } },
    ...
    async getMyCreatedTrips(userId, filters) {
        const where = {
            createdBy: userId,
        };
        if (filters.status) {
    ...
    async getMyJoinedTrips(userId, filters) {
        const where = {
    ...
    async getTripParticipants(tripId, userId) {
        const trip = await prisma.trip.findUnique({
            where: { id: tripId },
