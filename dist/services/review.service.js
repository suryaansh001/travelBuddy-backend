...
class ReviewService {
    ...
    // 6.6 Get My Reviews
    async getMyReviews(userId, type, options) {
    ...
    async getPendingReviews(userId, options) {
        ...
        const paginated = pendingReviews.slice((options.page - 1) * options.limit, options.page * options.limit);
        return {
    ...
    async getReviewSummary(userId) {
        ...
        for (const review of reviews) {
            const rating = Math.round(Number(review.overallRating));
            distribution[rating]++;
        }
        return {
