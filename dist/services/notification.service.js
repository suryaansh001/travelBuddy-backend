...
class NotificationService {
    ...
    async notifySwipeReceived(tripId, swiperId, tripCreatorId) {
    ...
    async notifyMatchConfirmed(matchId, tripId, userId) {
    ...
    async notifyTripUpdate(tripId, message) {
    ...
    async notifyTripCancelled(tripId, cancelledBy) {
        const trip = await prisma.trip.findUnique({
            where: { id: tripId },
            include: {
    ...
    async sendReviewReminder(userId, tripId) {
        const trip = await prisma.trip.findUnique({
            where: { id: tripId },
        });
        if (trip) {
    ...
    async sendPaymentReminder(userId, matchId, tripId, amount) {
        await this.createNotification({
            userId,
