...
export class MatchService {
    // 4.6 Accept Match
    async acceptMatch(userId, matchId, seatsRequested) {
        ...
        if (match.trip.availableSeats < seatsRequested) {
            throw new AppError(`Only ${match.trip.availableSeats} seat(s) available`, 400);
        }
    ...
    // 4.7 Reject Match
    async rejectMatch(userId, matchId, reason) {
        const match = await prisma.match.findUnique({
            where: { id: matchId },
            include: {
                trip: {
    ...
    async cancelMatch(userId, matchId, reason) {
        const match = await prisma.match.findUnique({
            where: { id: matchId },
            include: {
                trip: {
        ...
        const departureDateTime = new Date(`${match.trip.departureDate}T${match.trip.departureTime}`);
    ...
    // Get My Matches
    async getMyMatches(userId, filters) {
        // Build where clause
        const where = {
        ...
        return {
            matches: matches.map((match) => ({
                ...
                otherUser: match.tripCreatorId === userId
                    ? match.matchedUser
                    : match.tripCreator,
    ...
    async getPendingMatchesCount(userId) {
