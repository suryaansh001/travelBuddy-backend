...
import { swipeOnTripSchema, getSwipesQuerySchema, creatorSwipeSchema, getMySwipesQuerySchema, getMatchStatusSchema, } from '../validators/swipe.validator.js';
export class SwipeController {
    // 4.1 Swipe on Trip
    async swipeOnTrip(req, res, next) {
        try {
            const validated = swipeOnTripSchema.parse({
                body: req.body,
                params: req.params,
            });
            const result = await swipeService.swipeOnTrip(req.user.id, validated.params.tripId, validated.body.direction, validated.body.introductionMessage);
            res.status(201).json({
                success: true,
                data: result,
            });
    ...
    async getSwipesOnTrip(req, res, next) {
        try {
            const validated = getSwipesQuerySchema.parse({
                query: req.query,
                params: req.params,
            });
            const result = await swipeService.getSwipesOnTrip(req.user.id, validated.params.tripId, {
    ...
    async creatorSwipeOnUser(req, res, next) {
        try {
            const validated = creatorSwipeSchema.parse({
                body: req.body,
                params: req.params,
            });
            const result = await swipeService.creatorSwipeOnUser(req.user.id, validated.params.tripId, validated.params.userId, validated.body.direction);
            res.json({
                success: true,
                data: result,
            });
    ...
    // 4.4 Get My Swipes
    async getMySwipes(req, res, next) {
        try {
    ...
    async getMatchStatus(req, res, next) {
        try {
            ...
            const result = await swipeService.getMatchStatus(req.user.id, validated.params.tripId);
            res.json({
                success: true,
                data: result,
            });
    ...
    async getSwipeSummary(req, res, next) {
        try {
