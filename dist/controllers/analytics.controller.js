...
import { trackEventSchema, getTrendingRoutesSchema, suggestTripsSchema, platformAnalyticsSchema, collegeLeaderboardSchema, userBehaviorInsightsSchema, } from '../validators/analytics.validator.js';
// 9.1 Track Event
...
export const getUserMatchingScore = async (req, res, next) => {
    try {
        const userId = req.user.id;
...
export const getUserBehaviorInsights = async (req, res, next) => {
    try {
        const userId = req.user.id;
