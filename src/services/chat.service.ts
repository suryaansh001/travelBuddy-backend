...
class ChatService {
  ...
  async getOrCreateChatRoom(tripId: string, userId: string) {
  ...
  async getChatParticipants(tripId: string) {
    const trip = await prisma.trip.findUnique({
      where: { id: tripId },
      select: {
        createdBy: true,
    ...
    const participants = [
      { ...trip.creator, role: 'creator' as const },
      ...trip.matches.map(m => ({ ...m.matchedUser, role: 'participant' as const })),
    ];
  ...
  // 5.2 Send Message
  async sendMessage(roomId: string, userId: string, data: SendMessageData) {
    ...
    let payload: Record<string, any> = { read_by: [userId] };
  ...
  async getMessages(
    ...
    return {
      messages: resultMessages.reverse().map(msg => ({
        ...
        isRead: Array.isArray((msg.payload as any)?.read_by) 
          ? (msg.payload as any).read_by.includes(userId)
          : false,
      })),
      pagination: {
        hasMore,
  ...
  // 5.9 Flag Message
  async flagMessage(roomId: string, messageId: string, userId: string, reason: string) {
    const message = await prisma.chatMessage.findFirst({
      where: {
        id: messageId,
        roomId,
    ...
    await prisma.safetyReport.create({
      data: {
        ...
        type: 'chat_message',
        reason,
        description: `Flagged chat message: "${(message.payload as any)?.content || 'Non-text message'}"`,
        status: 'pending',
      },
    });
  ...
  // Get My Chat Rooms
  async getMyChatRooms(userId: string, options: { page: number; limit: number }) {
  ...
  async sendSystemMessage(roomId: string, content: string, eventType?: string, eventData?: Record<string, any>) {
    const chatRoom = await prisma.chatRoom.findUnique({
      where: { id: roomId },
    });
    ...
    const message = await prisma.chatMessage.create({
      data: {
        roomId,
        senderId: null as any, // System messages have no sender
        messageType: 'system',
        payload: {
          content,
