...
import type {
  ...
  BulkActionInput,
  ContentModerationSearchInput,
  ModerateContentInput,
  SystemLogsInput,
  AddCollegeWhitelistInput,
  UpdateCollegeWhitelistInput,
  CollegeWhitelistQueryInput,
} from '../validators/admin.validator.js';

// Type for admin log entry
...
export const getReports = async (
  ...
  reports: Array<{
    ...
    reportedUser: { id: string; fullName: string; email: string } | null;
    reportedTrip: { id: string; title: string | null } | null;
  }>;
...
export const updateReportStatus = async (
  ...
  const updateData: Prisma.SafetyReportUpdateInput = {
    status: data.status,
...
export const searchContent = async (
  params: ContentModerationSearchInput
): Promise<{
  content: Array<{
    id: string;
    type: 'trip' | 'message';
    ...
    return {
      content: messages.map((m) => ({
        ...
        content: typeof m.payload === 'object' && m.payload !== null && 'text' in m.payload 
          ? String((m.payload as { text?: string }).text || '[Media/System Message]')
...
export const moderateContent = async (
  adminId: string,
  contentType: 'trip' | 'message',
  contentId: string,
  data: ModerateContentInput
...
export const getSystemLogs = async (
  ...
  // Filter by criteria
  if (startDate) {
    logs = logs.filter((l: AdminLogEntry) => new Date(l.timestamp) >= startDate);
  }
  if (endDate) {
    logs = logs.filter((l: AdminLogEntry) => new Date(l.timestamp) <= endDate);
  }
  if (eventType) {
    logs = logs.filter((l: AdminLogEntry) => l.action.includes(eventType));
  }
  if (userId) {
...
// Type for college whitelist entry
interface CollegeWhitelistEntry {
  collegeName: string;
  isActive: boolean;
  addedAt: string;
  addedBy?: string;
...
export const getCollegeWhitelist = async (
  params: CollegeWhitelistQueryInput
): Promise<{
  colleges: Array<{
  ...
  if (search) {
    ...
    colleges = colleges.filter(
      (c) =>
        c.collegeDomain.toLowerCase().includes(searchLower) ||
...
export const addCollegeToWhitelist = async (
  ...
  const collegeData: CollegeWhitelistEntry = {
...
export const updateCollegeWhitelist = async (
  ...
  const collegeData = JSON.parse(existing) as CollegeWhitelistEntry;
