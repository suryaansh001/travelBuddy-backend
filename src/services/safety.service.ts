...
class SafetyService {
  ...
  async createReport(reporterId: string, data: CreateReportInput) {
    ...
    const report = await prisma.safetyReport.create({
      data: {
        ...
        emergencyLocation: data.emergencyLocation as any,
        status: 'pending',
      },
      include: {
        reportedUser: {
  ...
  private async handleEmergencyReport(reportId: string, reporterId: string) {
  ...
  // 7.2 Get My Reports
  async getMyReports(reporterId: string, options: { page: number; limit: number }) {
  ...
  // 7.3 Block User
  async blockUser(blockerId: string, data: BlockUserInput) {
  ...
  async getBlockedUsers(blockerId: string, options: { page: number; limit: number }) {
  ...
  async addEmergencyContact(userId: string, data: AddEmergencyContactInput) {
  ...
  async updateEmergencyContact(userId: string, contactId: string, data: Partial<AddEmergencyContactInput>) {
  ...
  // 7.10 Trigger SOS
  async triggerSOS(userId: string, data: TriggerSOSInput) {
    // Get user info
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: {
    ...
    const report = await prisma.safetyReport.create({
      data: {
        ...
        isEmergency: true,
        emergencyLocation: data.location as any,
