...
export class SwipeService {
  ...
  async getMySwipes(
    ...
    if (filters.status !== 'all') {
      results = results.filter(r => {
        if (filters.status === 'matched') return r.matchStatus.matched;
        if (filters.status === 'pending') return !r.matchStatus.matched && r.direction !== 'left';
        if (filters.status === 'rejected') return r.matchStatus.status === 'rejected';
        return true;
      });
    }
  ...
  async getMatchStatus(userId: string, tripId: string) {
  ...
  async getSwipeSummary(tripId: string) {
