...
export class UserService {
  ...
  async updateInterests(userId: string, data: UpdateInterestsInput) {
    // Validate interests against predefined list
    const validInterests = data.interests.filter(interest => 
      PREDEFINED_INTERESTS.includes(interest.toLowerCase() as typeof PREDEFINED_INTERESTS[number])
    );
  ...
  // Helper methods
  private async getCancellationRate(userId: string): Promise<number> {
