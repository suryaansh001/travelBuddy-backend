...
export class MatchController {
  // 4.6 Accept Match
  async acceptMatch(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async rejectMatch(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await matchService.rejectMatch(
        req.user!.id,
        validated.params.matchId,
  ...
  async getMatchDetails(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await matchService.getMatchDetails(
        req.user!.id,
        validated.params.matchId
      );
